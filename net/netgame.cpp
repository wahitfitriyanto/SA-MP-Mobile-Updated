...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char* szPlayerName, const char* szPass)
{
    ...
    if(pChatWindow)
	{
		pChatWindow->AddDebugMessage("Client Development By Febry");
		pChatWindow->AddDebugMessage("{FFFFFF}SA-MP {B9C9BF}" SAMP_VERSION "{FFFFFF} Mulai");
	}
}
...
void CNetGame::Process()
{
    ...
    {
        if(pChatWindow) pChatWindow->AddDebugMessage("Mencoba Menghubungkan Ke Server...");
    ...
    while(pkt = m_pRakClient->Receive())
    {
        ...
        switch(packetIdentifier)
        {
            ...
            case ID_CONNECTION_ATTEMPT_FAILED: {
                pChatWindow->AddDebugMessage("Server Tidak Merespon , Mencoba Kembali..");
                SetGameState(GAMESTATE_WAIT_CONNECT);
                break;
            }

            case ID_NO_FREE_INCOMING_CONNECTIONS:
                pChatWindow->AddDebugMessage("Slot Player Server Ini Penuh , Mencoba Menghubungkan Kembali...");
                SetGameState(GAMESTATE_WAIT_CONNECT);
                break;
            ...
            case ID_FAILED_INITIALIZE_ENCRIPTION:
                pChatWindow->AddDebugMessage("Gagal Memulai Enkripsi");
                break;

               /* case ID_CONNECTION_BANNED:
                pChatWindow->AddDebugMessage("Anda Telah Di Suspend Dari Server Ini.");
                break; */

            case ID_INVALID_PASSWORD:
                pChatWindow->AddDebugMessage("Password Server Salah!...");
                m_pRakClient->Disconnect(0);
                break;
...
{
    if(pChatWindow)
        pChatWindow->AddDebugMessage("Koneksi Server Telah Tutup.");
...
void CNetGame::Packet_ConnectionLost(Packet* pkt)
{
    ...
    if(pChatWindow)
        pChatWindow->AddDebugMessage("Jaringan Tidak Terdeteksi , Menghubungkan Kembali..");
...
{
    if(pChatWindow)
        pChatWindow->AddDebugMessage("Terhubung. Mencoba Masuk Kedalam Server...");
