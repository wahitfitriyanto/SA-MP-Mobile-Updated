...
#define UNSUPPORTED_SIZE 27
int unsupportedModel[UNSUPPORTED_SIZE] = {-1, 19902, 8324, 19596, 8293, 8292, 8310, 9191, 9190, 9189, 9188, 9187, 9186, 9185, 9184, 1978, 18879, 1755, 1977, 2785, 1284, 1302, 984, 2977, 1209, 1302, 956};
...
void ScrVehicleParamsEx(RPCParameters *rpcParams)
{
	...
	if(pNetGame && pNetGame->GetVehiclePool())
	{
		if(pNetGame->GetVehiclePool()->GetSlotState(VehicleId))
		{
			...
			// bonnet
			pNetGame->GetVehiclePool()->GetAt(VehicleId)->SetDoorOpenFlag(bonnet, 0);
			// trunk
			pNetGame->GetVehiclePool()->GetAt(VehicleId)->SetDoorOpenFlag(boot, 1);
			// object
			pNetGame->GetVehiclePool()->GetAt(VehicleId)->SetObjState(objective);
		}
	}
}
...
void ScrCreateObject(RPCParameters *rpcParams)
{
	...
	Log("Object Model: %d Total: %d", iModel, iTotalObjects);
		
	//Unsupported model will not execute
	for(int i = 0; i < UNSUPPORTED_SIZE; i++) 
	{
		if(iModel < 0 || iModel == unsupportedModel[i]) 
		{
			Log("Failed to load model %d", iModel);
			return;
		}
	}
	...
	CObjectPool *pObjectPool = pNetGame->GetObjectPool();
	pObjectPool->New(wObjectID, iModel, vecPos, vecRot, fDrawDistance);

	if(pObjectPool && iAttachedVehicle)
	{
		Log("Attaching object to vehicle %d", wObjectID);
	...
	Log("Success to load model %d", iModel);
}
...
pRakClient->RegisterAsRemoteProcedureCall(&RPC_ScrRemoveBuilding, ScrRemoveBuilding);
	...
	pRakClient->
RegisterAsRemoteProcedureCall(&RPC_ScrDeathMessage, ScrDeathMessage);
